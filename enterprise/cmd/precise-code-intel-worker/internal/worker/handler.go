...
func (h *handler) handle(ctx context.Context, upload store.Upload) (requeued bool, err error) {
	...
	return false, withUploadData(ctx, h.uploadStore, upload.ID, func(r io.Reader) (err error) {
		...
		if err := writeData(ctx, h.lsifStore, upload, repo, groupedBundleData); err != nil {
...
func writeData(ctx context.Context, lsifStore LSIFStore, upload dbstore.Upload, repo *types.Repo, groupedBundleData *precise.GroupedBundleDataChans) (err error) {
	...
	if err := tx.WriteDocumentationPages(ctx, upload, repo, groupedBundleData.DocumentationPages); err != nil {
